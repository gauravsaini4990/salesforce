        ...
        if (packageDir.ancestorVersion === 'NONE' || packageDir.ancestorId === 'NONE') {
            ...
            try {
                ...
                if (packageDir.ancestorVersion === 'HIGHEST') {
                    throw new core_1.SfdxError(messages.getMessage('errorNoMatchingAncestor', [packageDir.ancestorVersion, packageDir.package]), 'errorNoMatchingAncestor', [messages.getMessage('errorAncestorNotReleased', [packageDir.ancestorVersion])]);
                }
                else {
                    throw new core_1.SfdxError(messages.getMessage('errorNoMatchingAncestorId', [packageDir.ancestorId, packageDir.package]), 'errorNoMatchingAncestorId', [messages.getMessage('errorAncestorNotReleased', [packageDir.ancestorId])]);
                }
            }
        }
...
exports.generateScratchOrgInfo = async ({ hubOrg, scratchOrgInfoPayload, nonamespace, ignoreAncestorIds, }) => {
    ...
    let sfdxProject;
    try {
        sfdxProject = await core_1.SfdxProjectJson.create({});
    }
    catch (e) {
