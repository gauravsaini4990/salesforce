...
class SourceIgnoredCommand extends command_1.SfdxCommand {
    ...
    async run() {
        try {
            ...
            if (error.code === 'ENOENT') {
                throw core_1.SfdxError.create('@salesforce/plugin-source', 'ignored_list', 'invalidSourcePath', [
                    this.flags.sourcepath,
                ]);
            }
    ...
    async statIgnored(filepath) {
        const stats = await core_1.fs.stat(filepath);
    ...
    async findIgnored(dir) {
        ...
        return (await core_1.fs.readdir(dir)).map((filename) => this.statIgnored(path.join(dir, filename)));
    }
