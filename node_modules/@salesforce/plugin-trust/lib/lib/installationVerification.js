...
const npmCommand_1 = require("../lib/npmCommand");
        ...
        if (stream) {
            ...
            stream.on('end', () => {
                if (!key.includes('-----BEGIN')) {
                    return reject(new core_1.SfdxError('The specified key format is invalid.', 'InvalidKeyFormat'));
                }
                return resolve(key);
            });
            stream.on('error', (err) => {
        ...
        codeVerifierInfo.dataToVerify.on('end', () => {
            ...
            codeVerifierInfo.signatureStream.on('end', () => {
                if (signature.byteLength === 0) {
                    return reject(new core_1.SfdxError('The provided signature is invalid or missing.', 'InvalidSignature'));
                }
                else {
...
class InstallationVerification {
    constructor(requestImpl, fsImpl) {
        ...
        this.fsImpl = fsImpl ? fsImpl : core_1.fs;
    ...
    setConfig(_config) {
        ...
        throw new core_1.SfdxError('the cli engine config cannot be null', 'InvalidParam');
    }
    /**
    ...
    setPluginNpmName(_pluginName) {
        ...
        throw new core_1.SfdxError('pluginName must be specified.', 'InvalidParam');
    }
    /**
    ...
    async verify() {
        ...
        return Promise.all([this.getSigningContent(npmMeta.signatureUrl), this.getSigningContent(npmMeta.publicKeyUrl)])
            ...
            if (e.code === 'DEPTH_ZERO_SELF_SIGNED_CERT') {
                throw new core_1.SfdxError('Encountered a self signed certificated. To enable "export NODE_TLS_REJECT_UNAUTHORIZED=0"', 'SelfSignedCert');
            }
            throw e;
        });
    }
            ...
            this.requestImpl(url, (err, response, responseData) => {
                if (err) {
                    ...
                    if (response && response.statusCode === 200) {
                        ...
                        return reject(new core_1.SfdxError(`A request to url ${url} failed with error code: [${
    ...
    async retrieveNpmMeta() {
        ...
        if (!npmMetadata.versions) {
            throw new core_1.SfdxError(`The npm metadata for plugin ${this.pluginNpmName.name} is missing the versions attribute.`, 'InvalidNpmMetadata');
        }
        ...
        if (!versionNumber) {
            ...
            if (distTags) {
                ...
                if (tagVersionStr && tagVersionStr.length > 0 && tagVersionStr.includes('.')) {
                    ...
                    throw new core_1.SfdxError(`The dist tag ${this.pluginNpmName.tag} was not found for plugin: ${this.pluginNpmName.name}`, 'NpmTagNotFound');
                }
            }
            else {
                throw new core_1.SfdxError('The deployed NPM is missing dist-tags.', 'UnexpectedNpmFormat');
            }
        }
        npmModule.npmMeta.version = versionNumber;
        if (!npmMetadata.sfdx) {
            throw new core_1.SfdxError('This plugin is not signed by Salesforce.com, Inc.', 'NotSigned');
        }
        else {
            if (!validSalesforceHostname(npmMetadata.sfdx.publicKeyUrl)) {
                throw new core_1.SfdxError(`The host is not allowed to provide signing information. [${npmMetadata.sfdx.publicKeyUrl}]`, 'UnexpectedHost');
            }
            else {
            ...
            if (!validSalesforceHostname(npmMetadata.sfdx.signatureUrl)) {
                throw new core_1.SfdxError(`The host is not allowed to provide signing information. [${npmMetadata.sfdx.signatureUrl}]`, 'UnexpectedHost');
            }
            else {
...
async function doPrompt(vconfig) {
    ...
    switch (shouldContinue.toLowerCase()) {
        case 'y':
            return;
        default:
            throw new core_1.SfdxError('The user canceled the plugin installation.', 'InstallationCanceledError');
    }
}
...
async function doInstallationCodeSigningVerification(config, plugin, verificationConfig) {
    try {
        ...
        if (!meta.verified) {
            throw new core_1.SfdxError("A digital signature is specified for this plugin but it didn't verify against the certificate.", 'FailedDigitalSignatureVerification');
        }
        ...
        if (err.name === 'NotSigned') {
            ...
            throw new core_1.SfdxError(err.message || 'The user canceled the plugin installation.');
        }
