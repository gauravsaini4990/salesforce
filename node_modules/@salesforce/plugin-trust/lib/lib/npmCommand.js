...
class NpmCommand {
    static runNpmCmd(cmd, options = {}) {
        ...
        if (npmCmdResult.code !== 0) {
            throw new core_1.SfdxError(npmCmdResult.stderr, 'ShellExecError');
        }
        return npmCmdResult;
    }
...
class NpmModule {
    ...
    show(registry) {
        ...
        if (showCmd.stdout === '') {
            throw new core_1.SfdxError(`Failed to find ${this.module}@${this.version} in the registry`, 'NpmError');
        }
        try {
    ...
    pack(registry, options) {
        try {
            ...
            throw new core_1.SfdxError(`Failed to fetch tarball from the registry: \n${sfdxErr.message}`, 'NpmError');
        }
        return;
    }
